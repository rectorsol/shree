...
class Role_model extends CI_Model
{
    ...
    {
        return $this->db->get_where("roles", array("id" => $id, "deleted" => 0))->row(0);
    }
    ...
    public function get_displayname()
    {
        ...
        return $this->db->get_where("roles", array("deleted" => 0))->result();
    }
     ...
     * @return mixed
     */
    public function getDetailWithPermissions()
    {
        $sql = 'SELECT roles.*, GROUP_CONCAT(permissions.name) AS permissions FROM roles
                LEFT JOIN permission_roles ON roles.id = permission_roles.role_id
                LEFT JOIN permissions ON permission_roles.permission_id = permissions.id
                GROUP BY roles.id';
